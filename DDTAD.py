...
def DDTAD(datas, types, K=20, c=0.001):
    ...
    c : float, optional (default=0.001)
        Truncation threshold for DCS similarity, prevents excessive similarity values from affecting results
    ...
    dcs = np.divide(intersection, union)
    dcs_c = np.clip(dcs, a_min=0, a_max=c)
    ...
    for i in range(n_samples):
        ...
        neighbor_sim = dcs_c[current_idx, neighbor_index]
